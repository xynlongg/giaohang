<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">
            ...
            Đơn hàng ngoại thành
        </h5>
        <div class="d-flex gap-2">
            <select id="nonLocalDistributorSelect" class="form-select" style="min-width: 250px;">
                <option value="">Chọn nhân viên ngoại thành...</option>
                @foreach($nonLocalDistributors as $distributor)
                <option value="{{ $distributor->id }}">
                    {{ $distributor->name }} ({{ $distributorOrderCounts[$distributor->id] ?? 0 }}/20 đơn)
                </option>
                @endforeach
            </select>
            <button id="assignNonLocalBtn" class="btn btn-primary" disabled>
                <i class="bi bi-person-check me-1"></i>
                Gán đơn (<span id="selectedNonLocalCount">0</span>)
            </button>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                <thead>
                    <tr>
                        <th width="40">
                            <input type="checkbox" class="form-check-input select-all" data-type="non-local">
                        </th>
                        <th>Mã đơn hàng</th>
                        <th>Người gửi</th>
                        <th>Người nhận</th>
                ...
                <tbody>
                    @forelse($nonLocalOrders as $order)
                    <tr>
                        <td>
